"""Arena Phase 4 runtime API.

The lobby router owns money and lifecycle before the fight. This module owns
only authenticated access to the Redis-backed combat session; settlement is a
later phase and is deliberately not performed here.
"""
...
async def post_arena_action(
    ...
    runtime = _runtime(request)
    try:
        return await runtime.submit_action(
            match,
            auth.user_id,
            body.action,
...
async def post_arena_reconnect(
    ...
    runtime = _runtime(request)
    try:
        return await runtime.reconnect(match, auth.user_id)
    except arena_session_service.ArenaSessionError as exc:
        raise _map_session_error(exc) from exc
...
async def post_arena_disconnect(
    ...
    runtime = _runtime(request)
    try:
        return await runtime.disconnect(match, auth.user_id)
    except arena_session_service.ArenaSessionError as exc:
        raise _map_session_error(exc) from exc
...
async def post_arena_forfeit(
    ...
    runtime = _runtime(request)
    try:
        return await runtime.forfeit(match, auth.user_id)
    except arena_session_service.ArenaSessionError as exc:
        raise _map_session_error(exc) from exc
