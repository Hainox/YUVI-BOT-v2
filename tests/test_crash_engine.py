...
def test_rtp_full_cashout_path_matches_measured_double_quantization_bias():
    ...
    напрямую `crash_point`).

    CI-регрессия 2026-08-07: первая версия этого теста держала узкий допуск
    (±1пп вокруг 0.975) на n=50_000 — при биномиальной дисперсии выплаты
    (~50% шанс на 0 или ~2×bet) стандартная ошибка среднего на этом n сама
    порядка ~0.45пп, т.е. допуск был всего ~2 сигма и статистически обязан
    был иногда ложно падать (что и произошло на реальном CI-прогоне: 0.9637
    против нижней границы 0.965). Живой множитель `live_mult` — константа
    для фиксированного `target` и НЕ зависит от `rng`, поэтому раньше
    пересчитывался впустую на каждой итерации; вынесен за цикл, и это
    освободившееся время потрачено на n на порядок больше — при n=1_000_000
    стандартная ошибка падает примерно до ~0.1пп, поэтому широкий (по
    абсолютной величине) допуск ниже — это ~15+ сигма, статистически
    неотличимо от "никогда не падает по шуму", но всё ещё ловит НАСТОЯЩУЮ
    регрессию (например, случайно восстановленное ROUND_DOWN->округление до
    ближайшего подняло бы RTP заметно ближе к идеализированным 0.98)."""
    rng = secrets.SystemRandom()
    target = Decimal("2.00")
    live_mult = crash_engine.multiplier_at(crash_engine.seconds_to_reach(target))
    bet = Decimal(100)
    n = 1_000_000
    ...
    # ±1.5пп вокруг него на n=1_000_000 — многосигмовый запас (см. докстринг
    # выше), но всё ещё меньше идеализированных 0.98.
    assert Decimal("0.960") < empirical_rtp < Decimal("0.990"), (
