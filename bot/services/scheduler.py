...
через casino_service.register_blackjack_timeouts (план 04.1-03), и
mean-reversion тик AMM-пула фермы CP<->ювик (amm_mean_reversion, interval
10м, D-03) через clicker_service.register_amm_tick (план 04.1-05). Импорты
...
def setup_jobs(bot: Bot) -> None:
    ...
    блэкджека (план 04.1-03, blackjack_timeouts, interval 30с, D-07/D-08) и
    mean-reversion тик AMM-пула фермы (план 04.1-05, amm_mean_reversion,
    interval 10м, D-03). Ленивые импорты — эти модули появились в более
