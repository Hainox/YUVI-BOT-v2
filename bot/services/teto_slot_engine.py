...
# Порог фриспинов на СУММЕ ПЛОЩАДЕЙ скаттер-блоков (`teto_tumble.
# count_scatter_blocks`, разворот 2026-08-06 с "число блоков" на "сумма их
# клеток") — было 5 при подсчёте БЛОКОВ, поднято до 6 при перекалибровке RTP
# после разворота (см. историю и цифры в комментарии `teto_megablock.WEIGHTS`
# — сюда не дублируем, чтобы числа не разъехались за две калибровки). Сам
# факт "порог теперь считается по клеткам, а не по числу блоков" — не косметика:
# один скаттер-мегаблок area>=6 (например 2x3) теперь САМ ПО СЕБЕ пробивает
# порог, тогда как раньше требовалось 5+ ОТДЕЛЬНЫХ блоков одновременно на
# доске (структурно куда более редкое событие).
FREESPIN_SCATTER_MIN = 6
...
def _compute_freespins_awarded(scatter_count: int) -> int:
    ...
    0` — буквально из roadmap.md, ОТКРЫТАЯ формула. Намеренно НЕ 3-кейсовый
    lookup вроде `slot_engine._freespins_for` (тот класс бага уже найден и
    задокументирован в roadmap.md для Azumanga — здесь не повторяем:
    скаттер-мегаблок может закрыть 6-16 клеток одним блоком, поэтому
    подстановка фиксированного числа фриспинов независимо от `scatter_count>=
    FREESPIN_SCATTER_MIN` занижала бы выплату).

    ⚠ НЕ ТРОГАТЬ без отдельного owner-решения (открытый вопрос, зафиксирован
    2026-08-06 при перекалибровке RTP после разворота `count_scatter_blocks`
    на сумму площадей — см. её докстринг в `teto_tumble.py`): с площадным
    подсчётом `scatter_count` для реального триггера обычно лежит в диапазоне
    6-20 (наблюдалось до ~20 на Monte-Carlo прогонах по 300k спинов калибровки,
    см. `teto_megablock.WEIGHTS`), т.е. формула буквально начисляет 6-20
    фриспинов за спин, тогда как раньше (подсчёт блоков) типичный триггер был
    ~5-8. `FREESPINS_HARD_CAP=100` остаётся с большим запасом (наблюдённый
    максимум ~20, запас >=5x) — хардкап НЕ является риском сейчас, но сама
    формула "фриспины == scatter_count" была рассчитана на МЕДЛЕННО растущий
    scatter_count (блок = 1), а не на счёт, который одним мегаблоком прыгает
    сразу на 16. Пересмотр формулы (например, деление на константу или
    lookup-таблица по диапазонам scatter_count) — ОТДЕЛЬНОЕ решение, которое
    эта перекалибровка сознательно НЕ принимает: калибровка от 2026-08-06
    компенсировала возросший размер выплаты понижением ЧАСТОТЫ триггера
    (`WEIGHTS[SCATTER_ID]` ниже + этот порог), а не изменением того, сколько
    фриспинов даёт сам триггер."""
