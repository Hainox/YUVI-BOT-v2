...
`bot: TagBotLike` (не конкретный aiogram `Bot`) — этот модуль обращается
ровно к одному методу Bot API (`set_chat_member_tag`), поэтому параметр
типизирован узким Protocol (python-patterns: duck typing), а не самим
`Bot`: реальный aiogram `Bot` структурно ему уже удовлетворяет (никаких
изменений в bot-хендлерах не требуется), но это же позволяет `api`-процессу
(TAG-02 Mini App роуты, `api/routes/tags.py`) передать сюда свой
raw-httpx-адаптер (`api/tag_apply.py`) — процесс `api` намеренно не держит
aiogram `Bot`-инстанс ради одного REST-вызова (тот же архитектурный запрет,
что уже задокументирован в `api/telegram_client.py`/`api/duel_mute.py`).
"""
...
class TagBotLike(Protocol):
    """Утиный тип для единственного Bot API вызова, который делает этот
    модуль — см. модульный докстринг выше."""
...
async def _set_tag(bot: TagBotLike, chat_id: int, user_id: int, title: str) -> None:
...
async def _demote(bot: TagBotLike, chat_id: int, user_id: int) -> None:
...
async def clear_title(bot: TagBotLike, session: AsyncSession, chat_id: int, user_id: int) -> None:
...
async def expire_due(bot: TagBotLike, session: AsyncSession) -> int:
...
def register_title_expiry(scheduler: AsyncIOScheduler, bot: TagBotLike) -> None:
