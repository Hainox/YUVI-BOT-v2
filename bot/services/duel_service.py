...
async def create_duel(
    ...
    WR-01 (04.2-REVIEW): self-duel гард жил только в `bot/handlers/duel.py`
    (клиентская UX-проверка на entry point бота) и не был перенесён на
    Mini App API / общий service-слой — через API можно было вызвать
    самого себя, что после `accept_duel` детерминированно давало
    winner_id == loser_id == <self> (мут "победителя"). Гард теперь живёт
    здесь один раз, чтобы оба входа (бот-команда и API-роут) унаследовали
    его бесплатно; проверка в `bot/handlers/duel.py` остаётся как
    быстрый UX-путь до похода в БД."""
    if opponent_id == challenger_id:
        raise DuelError("Нельзя вызвать на дуэль самого себя")
